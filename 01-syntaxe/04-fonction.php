...
function dump(array $arr, callable $func): void{
    // Le type callable, indique que cet argument peut être appelé.
    foreach($arr as $a){
...
// elle sera utilisé par la fonction dump.
dump($tab, function($x){ echo $x;});
// je donne une fonction fleché en callback de ma fonction dump()
dump($tab, fn($x)=>var_dump($x));

$superFonction = function($x){
    print($x);
    /* Attention, cette façon de déclarer une fonction, ne peut 
    pas être appelée avant sa déclaration. */
};
